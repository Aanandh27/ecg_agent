...
# ─────────────────────────────────────────
# GLOBAL CSS — Hospital Information System Theme
# Dark navy + clinical blue + crisp white
# Font: IBM Plex Sans (clinical, technical, readable)
# ─────────────────────────────────────────
st.markdown("""
...
st.markdown("""
<div style="
    background: linear-gradient(90deg, #0d1425 0%, #0f1f3d 100%);
...
with st.sidebar:
    ...
    <div style="margin-top:8px;">
        <a href="https://aistudio.google.com" target="_blank" style="
            color:#38bdf8; font-size:0.78rem; text-decoration:none;
        ">↗ Get free API key</a>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="color:#64748b; font-size:0.82rem; line-height:1.8;">
        ...
        <div style="margin-bottom:6px;">③ Click Analyse ECG</div>
...
def render_parameter_table(result):
    ...
    <style>
        ...
        .pt thead th {
            ...
            color:#38bdf8; font-size:0.72rem; font-weight:600;
    ...
    <table class="pt">
        <thead><tr>
            <th>Parameter</th><th>Measured Value</th>
            <th>Normal Range</th><th>Status</th>
        </tr></thead><tbody>
    """
    for r in rows:
...
def render_clinical_findings(result):
    ...
    <style>
        ...
        .cf-card {
            ...
            background:#0d1425; transition:all 0.15s;
        }
        .cf-card:hover { background:#111f3a; }
        .cf-card.abnormal {
            border-color:#7f1d1d; background:#1a0808;
        ...
        .warn-icon { margin-left:8px; font-size:0.85rem; }
    </style>
    """
    for key, label in CLINICAL_FIELDS:
        ...
        warn = '<span class="warn-icon">⚠️</span>' if is_bad else ""
        cards_html += f"""
...
with main:
    ...
    if not uploaded_file and not api_key:
        ...
        ">
            ...
            <h1 style="color:#e2e8f0;font-size:1.8rem;font-weight:700;margin:0 0 8px 0;">
                CardioScan AI
            </h1>
            ...
            <p style="color:#64748b;font-size:0.95rem;max-width:480px;margin:0 auto;line-height:1.7;">
                ...
                clinical measurements, rhythm interpretation, and risk assessment.
            </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    if uploaded_file and not api_key:
        st.markdown("""
        <div style="background:#1c1000;border:1px solid #92400e;border-radius:10px;padding:14px 18px;
            color:#fbbf24;font-size:0.88rem;margin-top:12px;">
    ...
    if uploaded_file and api_key:
        if st.button("🔍 Run ECG Analysis", use_container_width=True, type="primary"):
            ...
            try:
                ...
                st.markdown(f"""
                <div style="
                    background:linear-gradient(90deg,#0d1f3c,#0f2a4a);
                    border:1px solid #1e3a5f; border-radius:12px;
                    padding:20px 28px; margin:24px 0 0 0;
                    display:flex; align-items:center; gap:24px;
                ">
                    <div style="
                        width:52px;height:52px;background:linear-gradient(135deg,#0ea5e9,#0369a1);
                        border-radius:50%;display:flex;align-items:center;justify-content:center;
                        font-size:22px;flex-shrink:0;
                    ">👤</div>
                    <div style="flex:1;">
                        <div style="color:#e2e8f0;font-size:1.2rem;font-weight:700;">
                            {result.get('patient_name','Unknown Patient')}
                        </div>
                    ...
                    <div style="text-align:right;flex-shrink:0;">
                        ...
                        <div style="color:#94a3b8;font-size:0.85rem;font-weight:500;margin-top:2px;">
                            {result.get('recorded_on','—')}
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                # ── PARAMETER TABLE + GAUGE SIDE BY SIDE ──
                ...
                with right:
                    ...
                    st.markdown(f"""
                    <div style="
                        background:{urg_cfg[0]};border:1px solid {urg_cfg[2]};
                        border-radius:10px;padding:14px 18px;text-align:center;margin-top:8px;
                    ">
                    ...
                    <div style="background:#0d1425;border:1px solid #1e3a5f;border-radius:10px;padding:16px 18px;margin-top:12px;">
                        <div style="color:#38bdf8;font-size:0.72rem;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:8px;">
                            Primary Rhythm
                        </div>
                ...
                with su:
                    ...
                    <div style="background:#052e16;border:1px solid #166534;border-left:3px solid #22c55e;
                        ...
                        <div style="color:#bbf7d0;font-size:0.88rem;line-height:1.7;">
                            {result.get('overall_condition','No summary available')}
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                st.markdown("""
                <div style="
                    background:#0d1425;border:1px solid #1e3a5f;border-radius:10px;
                    padding:14px 20px;margin-top:16px;text-align:center;
                    color:#475569;font-size:0.78rem;line-height:1.6;
                ">
                    ⚠️ This report is generated by an AI system and is intended for educational and demonstration purposes only.
                    All findings must be reviewed and confirmed by a qualified medical professional before any clinical decision is made.
                </div>
                """, unsafe_allow_html=True)
